############################
# LiteBlob internal interface contract
#
...
#      - Numbers, Strings, and AbstractLiteBlob instances are returned
...
#          * we materialize a fresh `LiteBlob(OrderedDict(x), Dict())`
#            so the blob holds its own stable structure.
#
#    Subtypes of `AbstractLiteBlob` MAY specialize `_liteobj` for their
...
#               Vector{<:AbstractLiteBlob}     # no vector-of-blobs
...
#               length(v) ≤ 10
#           via `_islite(b::AbstractLiteBlob, v::Vector)`.
#
...
#    `haskey`, `iterate`, etc. directly on an `AbstractLiteBlob`.
#
...
## ---.- .-- - .-. .-. -.- .-. -.-.-
# MARK: islite(..., type)
_islite(::Type{<:AbstractLiteBlob}, ::Type{<:Number}) = true
_islite(::Type{<:AbstractLiteBlob}, ::Type{String}) = true
_islite(L::Type{<:AbstractLiteBlob}, ::Type{Vector{T}}) where {T} = _islite(L, T)
_islite(::Type{<:AbstractLiteBlob}, ::Type{<:AbstractLiteBlob}) = true

# closing world
_islite(::Type{<:AbstractLiteBlob}, ::Type{<:Any}) = false

## ---.- .-- - .-. .-. -.- .-. -.-.-
# MARK: islite(..., object)
# _islite(b::AbstractLiteBlob, v) = _islite(typeof(b), typeof(v))

function _islite(b::AbstractLiteBlob, v::Vector) 
    length(v) > 100 && return false
    return _islite(typeof(b), eltype(v))
end

function _islite(x::AbstractLiteBlob, d::AbstractDict)
...
# closing world
_islite(::AbstractLiteBlob, ::Any) = false

## ---.- .-- - .-. .-. -.- .-. -.-.-
# MARK: _liteobj
# Default mustly pass original object if it is lite
_liteobj(::AbstractLiteBlob, x::Number) = x
_liteobj(::AbstractLiteBlob, x::String) = x
# TODO/TAI maybe add lenght lim
function _liteobj(b::AbstractLiteBlob, x::Vector)
    _islite(b, x) || error("Non-lite vector")
    return copy(x)
end

_liteobj(::AbstractLiteBlob, x::AbstractLiteBlob) = x

# default fallback to LiteBlob
function _liteobj(b::AbstractLiteBlob, x::AbstractDict)
    _islite(b, x) || error("Non-lite dictionary")
    LiteBlob(OrderedDict(x), Dict())
end

# fallback
_liteobj(::AbstractLiteBlob, x::Any) =
...
# get / set by key
function _lite_getindex(x::AbstractLiteBlob,
    k::String
)
...
function _lite_haskey(x::AbstractLiteBlob, k::String)
...
## ---.- .-- - .-. .-. -.- .-. -.-.-
# MARK: lite_get
function _lite_get(x::AbstractLiteBlob, k::String, default)
    return get(__depot__(x), k, default)
end
function _lite_get(f::Function, x::AbstractLiteBlob, k::String)
...
## ---.- .-- - .-. .-. -.- .-. -.-.-
# MARK: lite_delete!
function _lite_delete!(x::AbstractLiteBlob, k::String)
    delete!(__depot__(x), k)
end

## ---.- .-- - .-. .-. -.- .-. -.-.-
# MARK: lite_empty!
function _lite_empty!(x::AbstractLiteBlob)
    empty!(__depot__(x))
end
...
function _lite_merge!(x::AbstractLiteBlob, it)
...
## ---.- .-- - .-. .-. -.- .-. -.-.-
# MARK: lite_length
function _lite_length(x::AbstractLiteBlob)
    return length(__depot__(x))
end

## ---.- .-- - .-. .-. -.- .-. -.-.-
# MARK: lite_keys
function _lite_keys(x::AbstractLiteBlob)
    return keys(__depot__(x))
end

## ---.- .-- - .-. .-. -.- .-. -.-.-
# MARK: lite_values
function _lite_values(x::AbstractLiteBlob)
    return values(__depot__(x))
end

## ---.- .-- - .-. .-. -.- .-. -.-.-
# MARK: lite_pairs
function _lite_pairs(x::AbstractLiteBlob)
...
function _lite_in(k::String, x::AbstractLiteBlob)
    return in(k, keys(x))
end
...
function _lite_iterate(x::AbstractLiteBlob)
    return iterate(pairs(__depot__(x)))
end

## ---.- .-- - .-. .-. -.- .-. -.-.-
# MARK: lite_iterate
function _lite_iterate(x::AbstractLiteBlob, state)
...
function _lite_eltype(::Type{<:AbstractLiteBlob})
...
# MARK: lite_nthkey
# nth key (1-based)
function _lite_nthkey(x::AbstractLiteBlob, i0::Integer)
    i = 1
...
# MARK: lite_nthpair
# nth pair
function _lite_nthpair(x::AbstractLiteBlob, i0::Integer)
    i = 1
...
## ---.- .-- - .-. .-. -.- .-. -.-.-
# MARK: lite_json
function _lite_json(x::AbstractLiteBlob)
...
function _lite_jsonline(x::AbstractLiteBlob; esc_newline=false)
